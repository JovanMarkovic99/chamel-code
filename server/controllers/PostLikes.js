...
router.post("/:id", protected, async (req, res, next) => {
    try {
        ...
        const doc = (req.body.like == 0) ?
                await PostLikes.findOneAndDelete({ postId: req.params.id, username: req.user.username }) :
                await PostLikes.findOneAndUpdate({ postId: req.params.id, username: req.user.username }, { like: req.body.like }, { upsert: true });
        res.sendStatus(200);
...
router.get("/", protected, async (req, res, next) => {
    try {
        const likes = await PostLikes.find({ username: req.user.username });
        res.send(likes);
    } catch (err) {
        next(err);
    }
