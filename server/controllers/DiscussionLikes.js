...
router.post("/:id", protected, async (req, res, next) => {
    try {
        ...
        const doc = (req.body.like == 0) ? 
                await DiscussionLikes.findOneAndDelete({ discussionId: req.params.id, username: user.username }) : 
                await DiscussionLikes.findOneAndUpdate({ discussionId: req.params.id, username: user.username }, { like: req.body.like }, { upsert: true });
        res.sendStatus(200);
